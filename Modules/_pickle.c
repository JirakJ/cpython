...
static PyTypeObject pickle_error_obj;
static PyObject *pickle_error = (PyObject *)&pickle_error_obj;
static PyTypeObject pickling_error_obj;
static PyObject *pickling_error = (PyObject *)&pickling_error_obj;
static PyTypeObject unpickling_error_obj;
static PyObject *unpickling_error = (PyObject *)&unpickling_error_obj;
...
typedef struct {
    /* Types */
    PyTypeObject *Pickler;
    PyTypeObject *Unpickler;
...
/* Given a class object, find its per-module state. */
static PickleState *
_Pickle_StateFromType(PyTypeObject *cls)
{
    PyObject *module = PyType_GetModule(cls);
    if (!module) {
        return NULL;
    }
    if (PyModule_GetDef(module) != &_picklemodule) {
...
typedef struct {
    ...
    PickleState *module_state;  /* Module state */
} Pdata;
...
static PyObject *
Pdata_New(PickleState *module_state)
{
    Pdata *self;
...
{
    PyErr_SetString(unpickling_error,
                    self->mark_set ?
...
_Pickler_New(PyObject *module)
{
    ...
    st = _Pickle_GetState(module);
    if (st == NULL)
        return NULL;

    self = PyObject_GC_New(PicklerObject, st->Pickler);
    if (self == NULL)
        return NULL;
...
_Unpickler_New(PyObject *module)
{
    ...
    self = PyObject_GC_New(UnpicklerObject, st->Unpickler);
    if (self == NULL)
        return NULL;
...
memo_get(PicklerObject *self, PyObject *key)
{
    ...
    if (!self->bin) {
        ...
        if (*value < 256) {
            ...
            PyErr_SetString(pickling_error,
...
static int
fix_imports(PicklerObject *self, PyObject **module_name, PyObject **global_name)
{
    PyObject *key;
    PyObject *item;
    PickleState *st = self->module_state;
...
save_global(PicklerObject *self, PyObject *obj, PyObject *name)
{
    ...
    if (self->proto >= 2) {
        ...
        if (code <= 0 || code > 0x7fffffffL) {
            if (!PyErr_Occurred())
                PyErr_Format(pickling_error, "Can't pickle %R: extension "
  ...
  gen_global:
        ...
        if (self->proto >= 4) {
            ...
            if (self->proto < 3 && self->fix_imports) {
                if (fix_imports(self, &module_name, &global_name) < 0) {
                    goto error;
                }
            }
...
save_reduce(PicklerObject *self, PyObject *args, PyObject *obj)
{
    ...
    if (size < 2 || size > 5) {
        PyErr_SetString(pickling_error, "tuple returned by "
    ...
    if (!PyCallable_Check(callable)) {
        PyErr_SetString(pickling_error, "first item of the tuple "
                        "returned by __reduce__ must be callable");
        return -1;
    }
    if (!PyTuple_Check(argtup)) {
        PyErr_SetString(pickling_error, "second item of the tuple "
    ...
    if (listitems == Py_None)
        ...
        PyErr_Format(pickling_error, "fourth element of the tuple "
    ...
    if (dictitems == Py_None)
        ...
        PyErr_Format(pickling_error, "fifth element of the tuple "
                     "returned by __reduce__ must be an iterator, not %s",
    ...
    if (use_newobj_ex) {
        ...
        /* Sanity checks. */
        if (Py_SIZE(argtup) < 1) {
            PyErr_SetString(pickling_error, "__newobj__ arglist is empty");
            return -1;
        }

        cls = PyTuple_GET_ITEM(argtup, 0);
        if (!PyType_Check(cls)) {
            PyErr_SetString(pickling_error, "args[0] from "
...
static PyType_Slot Pickler_slots[] = {
    {Py_tp_doc, _pickle_Pickler___init____doc__},
...
static PyType_Spec Pickler_spec = {
    "_pickle.Pickler",
...
marker(UnpicklerObject *self)
{
    ...
    if (self->num_marks < 1) {
        PyErr_SetString(unpickling_error, "could not find MARK");
        return -1;
    }
...
bad_readline(void)
{
    PyErr_SetString(unpickling_error, "pickle data was truncated");
    return -1;
}
...
load_dict(UnpicklerObject *self)
{
    ...
    if ((j - i) % 2 != 0) {
        PyErr_SetString(unpickling_error, "odd number of items for DICT");
        Py_DECREF(dict);
        return -1;
    }
...
load_newobj(UnpicklerObject *self)
{
    ...
    if (cls == NULL)
        goto error;
    if (!PyType_Check(cls)) {
        PyErr_SetString(unpickling_error, "NEWOBJ class argument "
                        "isn't a type object");
        goto error;
    }
    if (cls->tp_new == NULL) {
...
load_stack_global(UnpicklerObject *self)
{
    ...
    if (module_name == NULL || !PyUnicode_CheckExact(module_name) ||
        ...
        PyErr_SetString(unpickling_error, "STACK_GLOBAL requires str");
        Py_XDECREF(global_name);
        Py_XDECREF(module_name);
        return -1;
    }
...
load_extension(UnpicklerObject *self, int nbytes)
{
    ...
    if (code <= 0) {            /* note that 0 is forbidden */
        ...
        PyErr_SetString(unpickling_error, "EXT specifies code <= 0");
        return -1;
    }
...
load_build(UnpicklerObject *self)
{
    ...
    if (state != Py_None) {
        ...
        if (!PyDict_Check(state)) {
            PyErr_SetString(unpickling_error, "state is not a dictionary");
            goto error;
        }
...
static PyType_Slot Unpickler_slots[] = {
    {Py_tp_dealloc, Unpickler_dealloc},
    {Py_tp_doc, _pickle_Unpickler___init____doc__},
...
static PyType_Spec Unpickler_spec = {
    "_pickle.Unpickler",
